 ...
 */
function readContents(fileContents, args = {}) {
    ...
    for (let line of lines) {
        ...
        if (args.newMessageRegEx.test(line)) {
            ...
            if (aggregate.length > 0) {
                const newActivities = readCommandsFromAggregate(args, aggregate);
                if (newActivities) {
 ...
 */
function readCommandsFromAggregate(args, aggregate) {
    const newActivities = [];
    ...
    while ((result = commandRegExp.exec(aggregate))) {
        ...
        if (type) {
            ...
            case activityfield.attachment:
                addAttachment(currentActivity, rest);
                break;
...
function fileListContent(url) {
    var i = fileList.length;
    while (i--) {
       if (fileList[i].file === url) {
           return fileList[i].content;
       }
    }
    return null;
}
...
function getHashCode(contents) {
    return "hash"; //TODO crypto.createHash('sha1').update(contents).digest('base64')
}